...
export default {
  ...
  getInstance() {
    ...
    this.instance.img_sets.apple.path =
      "https://cdn.jsdelivr.net/npm/emoji-datasource-messenger@4.1.0/img/messenger/64/";
  ...
  convertEmoji(input) {
    return this.getInstance()
      .replace_unified(input)
      .replace(
        /<span class="([^"]+)" style="background-image:url\(([^"]+)\)" data-codepoints="([a-z0-9]+)"><\/span>/g,
  ...
  convertEmojiStr(input) {
    ...
    return input.replace(
      /<span contenteditable="false" class="([^"]+)" style="background-image:url\(([^"]+)\)" data-codepoints="([a-z0-9]+)"><\/span>/g,
      regexStr => {
