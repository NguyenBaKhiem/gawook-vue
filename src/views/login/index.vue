...
export default {
  ...
  methods: {
    ...
    async handleLogin() {
      ...
      if (this.errors.any() || this.loading) {
        return;
      }
      this.resetError();
      this.loading = true;
      window.axios
        .get("http://gawook-backend.io:8000/api/csrf-cookie")
        .then(
          function() {
            rf.getRequest("UserRequest")
              ...
              .then(response => {
                this.loading = false;
                AuthenticationUtils.login(response.data);
              ...
              .catch(error => {
                ...
                Message.error(
                  this.$t(this.errors.first("error")) ||
                    this.$t("auth.unknowError")
                );
              });
          }.bind(this)
        );
