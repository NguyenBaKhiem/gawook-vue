...
Vue.mixin({
  ...
  methods: {
    ...
    _appendErrors(errors) {
      _.forEach(errors, (error, key) => {
        const message = window.i18n.te(_.head(error))
          ? window.i18n.t(_.head(error), {
              attribute: window.i18n.t(`validation.attributes.${key}`)
            })
          : window.i18n.t(`validation.${_.head(error)}`, {
              attribute: window.i18n.t(`validation.attributes.${key}`)
            });
        this.errors.add({
          field: key,
    ...
    isRequiredGoogleAuthenticator() {
      const messageGoogleAuth = window.i18n.t("validation.required", {
        attribute: window.i18n.t(
          "validation.attributes.google_authenticator_code"
        )
      });
      return (
        this.errors.has("google_authenticator_code") &&
        this.errors.first("google_authenticator_code") === messageGoogleAuth &&
        this.errors.count() === 1
      );
    }
  }
...
Validator.prototype._formatErrorMessage = function _formatErrorMessage(
  ...
  return getFieldMessage(this.locale, field.name, rule.name, [
    name,
    params,
    data
  ]);
...
function getMessage(locale, rule, data) {
  if (hasDictionaryMessage(locale, rule)) {
    const _message =
      window.app.$validator.dictionary.container[locale].messages[rule];
  ...
  if (!window.i18n.te(path)) {
    if (hasDictionaryMessage(locale, "_default")) {
      const _message =
        window.app.$validator.dictionary.container[locale].messages._default;
      return typeof _message === "function" ? _message(...data) : _message;
    }
